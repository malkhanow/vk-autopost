...
def openers_rule():
    ...
    return (
        ...
        "Начинай пост живо и по-разному — с действия, с наблюдения, "
        "с конкретной детали, с реплики, с описания ситуации. "
    )
...
def build_post(client, rubric, photo_path=None):
    ...
    import random
    # Если в конфиге клиента задан post_format — используем его жёстко.
    # Иначе — случайный выбор из стандартного набора для разнообразия.
    fixed_format = (client.get("post_format") or "").strip()
    if fixed_format:
        formats = fixed_format
    else:
    ...
    system = (
            ...
            "Каждый пост должен отличаться от предыдущих по структуре, длине и форме подачи. "
            "Чередуй: короткие посты (3–4 предложения) и длинные (4–6 абзацев), "
            "истории и советы, вопросы к читателю и утверждения, один абзац и несколько. "
    ...
    # Модель иногда игнорирует стоп-лист. Одна повторная попытка с явным
    # указанием на конкретную фразу — дешевле, чем однообразная лента.
    if has_forbidden_opener(text):
        print("Пост начался с дежурной формулы — перегенерирую")
        try:
            retry = ai_text([
                {"role": "system", "content": system},
                {"role": "user", "content": (
                    user
                    + "\n\nПредыдущая попытка начиналась так: "
                    + " ".join((text or "").split()[:6])
                    + "… Это дежурная формула. Напиши пост заново, "
                    "начав совершенно иначе."
                )},
            ])
            if retry and not has_forbidden_opener(retry):
                text = retry
        except Exception as e:
