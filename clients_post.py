...
def append_cta(text, client, compact=False):
    ...
    compact=True — пост идёт с фотографией. У Telegram подпись к фото
    ограничена 1024 символами против 4096 у обычного сообщения: длинный CTA
    пробивает лимит, и пост разваливается на два сообщения — фото без
    подписи, текст отдельно. Если в карточке заполнено поле cta_short,
    в таких постах берётся оно. Не заполнено — поведение прежнее.

    ВАЖНО: короткий вариант пишет клиент, а не движок. Обрезать CTA
    автоматически нельзя: в нём бывают юридически обязательные строки
    (например, про противопоказания в медицинской рекламе).
    """
    cta = ""
    if compact:
        cta = (client.get("cta_short") or "").strip()
    if not cta:
...
RUBRIC_ANCHOR = {
    ...
    "photo_case": (
        # Название рубрики у каждого клиента своё («Фото работ», «Объекты и
        # документы», «Наши кейсы») — в общем движке его быть не должно.
        "Это рубрика с разбором работы по фотографии. Пост ОБЯЗАН разбирать "
        "одну конкретную "
...
def build_post(client, rubric, photo_path=None, state=None, client_id=""):
    ...
    if uses_vision and photo_path:
        ...
        if is_reviews:
            ...
            rubric_title = str(rubric.get("name") or "").strip()
            rubric_ref = ("рубрики «" + rubric_title + "»") if rubric_title \
                else "рубрики отзывов и результатов"
            context_parts = [
                # Имя рубрики берём из конфига клиента, а не из кода: у разных
                # клиентов она называется по-разному.
                "Ты получаешь изображение из " + rubric_ref + ".",
                ...
                "(результат работы, объект, встреча) — напиши тематический "
                "пост по этой рубрике без опоры на конкретику изображения.",
            ...
            context_parts = [
                ...
                # Верхняя граница подобрана под лимит подписи Telegram
                # (1024 символа). Замер на реальных постах: 6.45 символа на
                # слово, значит 130 слов ≈ 840 символов; с коротким CTA и
                # хештегами пост уходит одним сообщением вместе с фото.
                f"Объём: 100–130 слов. Пиши {style}",
    ...
    faq_pair = None
    if kind == "faq":
        # Вопросы из карточки клиента и темы рубрики — ОДИН пул, крутится
        # по кругу целиком. Раньше темы глушились, как только в поле faq
        # находилась хоть одна пара: у клиента с одним вопросом и пятнадцатью
        # темами рубрика выходила с одним и тем же вопросом бесконечно.
        # Пары идут первыми — у них есть готовый ответ клиента, это ценнее
        # темы, по которой модель отвечает сама.
        faq_pairs = parse_faq(client.get("faq"))
        rubric_topics = [str(t).strip()
                         for t in (rubric.get("topics") or []) if str(t).strip()]
        faq_pool = ([("pair", p) for p in faq_pairs] +
                    [("topic", t) for t in rubric_topics])
        faq_topic = ""
        if faq_pool:
            chosen = pick_from_cycle(faq_pool, state, client_id, "faq")
            if chosen and chosen[0] == "pair":
                faq_pair = chosen[1]
            elif chosen:
                faq_topic = chosen[1]
    ...
    elif kind == "faq" and faq_topic:
        # Готового вопроса у этой темы нет — модель формулирует его сама,
        # но строго в рамках темы, а не свободно.
        task_lines = [
            "ГЛАВНОЕ ТРЕБОВАНИЕ: " + RUBRIC_ANCHOR["faq"] +
            "\nТЕМА ЭТОГО ПОСТА: " + faq_topic +
            "\nСформулируй в первых двух предложениях вопрос, который реальный "
            "клиент задал бы именно по этой теме, и дальше ответь на него по "
            "существу. Уходить с темы в сторону запрещено."
        ]
    elif kind == "faq":
        task_lines = [
            "ГЛАВНОЕ ТРЕБОВАНИЕ: " + RUBRIC_ANCHOR["faq"] +
    ...
    # Для ЧЗВ тему уже задал пул выше — второй раз её подмешивать нельзя,
    # иначе в промпте окажутся две разные темы сразу.
    if topic and kind != "faq":
        task_lines.append(
