...
Пт — Как мы работаем (чётные недели) или Мир печати и дизайна
Сб — Идея для принта
Вс — Пост из 24
...
POST_HOW_TO_ORDER = """Как мы работаем:

1️⃣ Выбираешь изделие из каталога или приносишь своё
2️⃣ Уточняем размер, цвет и где будет принт
3️⃣ Делаем цифровой макет — смотришь, правим если нужно
4️⃣ После утверждения — 100% предоплата
5️⃣ Готово — забираешь сам или отправляем доставкой

Среднее время заказа — 48 часов после оплаты.

📩 Написать нам: [Telegram](https://t.me/customstudio_print) · [VK](https://vk.ru/cstmstd) · [Max](https://max.ru/u/f9LHodD0cOIznR4wgtmLlxX8R7URb3TY3TRqogCTCk3dPo8WVFJBykG2DTI)"""
...
def download_fixed_photo(yandex_path):
    """Скачивает фиксированный файл по точному пути. Возвращает путь или None."""
    try:
        return download_yandex_file(yandex_path)
    except Exception as e:
        print(f"Не удалось скачать {yandex_path}: {e}")
        return None
...
def main():
    ...
    if weekday == 0:
        ...
        if week_number % 2 == 0:
            print("Рубрика: Как мы работаем")
            text = POST_HOW_TO_ORDER
            photo_path = download_fixed_photo(HOW_TO_ORDER_PHOTO)
        else:
            print("Рубрика: Мир печати и дизайна")
            text, photo_path = generate_world_of_print()
