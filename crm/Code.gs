...
var RUBRIC_ANCHOR = {
  faq:
    ...
    'по существу. Пост-история, пост-наблюдение и пост-размышление здесь НЕ подходят.',
  docs:
    'Это рубрика про объекты и документы. Пост ОБЯЗАН разбирать один конкретный ' +
    'документ, справку, выписку или этап оформления: что это, зачем нужно, что ' +
    'в нём смотреть и что бывает, если этого не сделать. Общие рассуждения о ' +
    'рынке здесь НЕ подходят.',
  results:
    'Это рубрика результатов. Пост ОБЯЗАН показать один завершённый рабочий ' +
    'кейс по схеме: задача клиента — что было сделано — чем закончилось. ' +
    'Без сумм, сроков, процентов и имён.',
  inspiration:
    'Это рубрика про эмоцию и уют. Пост ОБЯЗАН опираться на ощущение от ' +
    'пространства или на спокойствие от порядка в делах. Инструкций, чек-листов ' +
    'и разборов документов здесь быть не должно.',
  observation:
    'Это рубрика наблюдений из практики. Пост ОБЯЗАН показать закономерность, ' +
    'которую видно только специалисту. Не инструкция и не реклама услуги.',
  tips:
    'Это рубрика советов. Пост ОБЯЗАН давать один применимый совет, который ' +
    'читатель может использовать сегодня же. Без общих слов.',
  howto:
    'Это рубрика «как сделать». Пост ОБЯЗАН вести читателя по шагам или ' +
    'критериям выбора. Абстрактные рассуждения здесь НЕ подходят.',
  service:
    'Это рубрика об услугах. Пост ОБЯЗАН объяснить одну услугу: что входит, ' +
    'кому подходит, чем помогает. Без цен и сроков.',
  news:
    'Это рубрика новостей и новинок. Пост ОБЯЗАН сообщить об одном конкретном ' +
    'изменении или поступлении и объяснить, чем оно полезно читателю.',
  team:
    'Это рубрика о команде. Пост ОБЯЗАН показать человека или подход к работе, ' +
    'а не перечислять услуги.',
  promo:
    'Это рубрика специальных предложений. Пост ОБЯЗАН описать суть предложения ' +
    'и кому оно подойдёт. Конкретные цифры и проценты не выдумывать.',
  before_after:
    'Это рубрика «до и после». Пост ОБЯЗАН показать контраст между исходным ' +
    'состоянием и результатом.',
  'default':
    'Пост ОБЯЗАН раскрыть одну мысль до конца и быть полезным читателю. ' +
    'Три общие фразы вместо одной доведённой мысли — не подходит.'
};
...
var RUBRIC_FORMATS = {
  faq: [
    ...
    { scene: false, text: 'Формат «два варианта». Вопрос первой строкой, затем сравни два способа его решить и объясни, какой ты советуешь и почему. 100–140 слов.' },
    { scene: true,  text: 'Формат «вопрос с консультации». Первая строка — вопрос, который прозвучал на встрече, дословно. Дальше — как автор на него отвечает. 90–130 слов.' }
  ],
  docs: [
  ...
  results: [
    ...
    { scene: false, text: 'Формат «нетипичный запрос». Опиши необычное требование клиента и как оно было закрыто. 90–130 слов.' },
    { scene: false, text: 'Формат «что удалось предотвратить». Расскажи, какую проблему нашли заранее и чем это помогло. 90–130 слов.' },
    { scene: true,  text: 'Формат «короткий кейс из практики». Одна работа от обращения до завершения, спокойно и по фактам. 90–130 слов.' }
  ],
  inspiration: [
    { scene: false, text: 'Формат «атмосфера». Начни с описания света, звука или ощущения, затем свяжи это с темой бизнеса. 80–120 слов.' },
    { scene: false, text: 'Формат «маленькая деталь». Возьми одну деталь и объясни, как она меняет впечатление. 80–120 слов.' },
    { scene: false, text: 'Формат «размышление». Спокойная мысль по теме бизнеса, без инструкций. 70–110 слов.' },
    { scene: false, text: 'Формат «сравнение настроений». Покажи контраст: как ощущается до и как после. 80–120 слов.' },
    { scene: true,  text: 'Формат «момент из работы». Опиши, как клиент понял, что нашёл своё. Тепло, без пафоса. 80–120 слов.' }
  ],
  observation: [
    ...
    { scene: false, text: 'Формат «что замечают не сразу». Возьми деталь, которую клиенты пропускают, и объясни, почему она важна. 90–130 слов.' },
    { scene: false, text: 'Формат «два типа клиентов». Опиши две разные модели поведения и что из них выходит. 90–130 слов.' },
    { scene: false, text: 'Формат «взгляд изнутри профессии». Не стандартный совет, а то, что видно только специалисту. 80–120 слов.' },
    { scene: true,  text: 'Формат «случай, который многое объясняет». Одна ситуация из практики и вывод из неё. 90–130 слов.' }
  ],
  tips: [
    { scene: false, text: 'Формат «один совет». Назови совет в первой строке, затем объясни, почему он работает. 80–120 слов.' },
    { scene: false, text: 'Формат «три пункта». Короткое вступление и три конкретных пункта. 90–130 слов.' },
    { scene: false, text: 'Формат «чего не делать». Назови распространённую ошибку и дай правильный вариант. 80–120 слов.' },
    { scene: false, text: 'Формат «как проверить себя». Дай читателю простой критерий, по которому он поймёт, всё ли у него в порядке. 80–120 слов.' },
    { scene: false, text: 'Формат «правило на всякий случай». Одно правило, которое пригодится не сегодня, но точно пригодится. 80–120 слов.' }
  ]
};
